...
from trainer import MyTrainer, inference
...
if __name__ == "__main__":
    ...
    parser.add_argument('--use_type', type=str, choices=['train', 'inference', "val"], default='train', help='Select the mode: train or inference')
    ...
    if args.use_type == 'train':
        ...
        logging.basicConfig(filename="val.log", level=logging.INFO, format='%(asctime)s - %(levelname)s - %(message)s')
        ...
        if args.filefolder_path + "/test.txt" is not None:
            ...
            test_list = []
            with open(args.filefolder_path + "/test.txt", "r") as f:
    ...
    elif args.use_type == 'val':
        logging.basicConfig(filename="val.log", level=logging.INFO, format='%(asctime)s - %(levelname)s - %(message)s')
        print(f"The data will be loaded from: {args.filefolder_path}")
        print(f"Validation will proceed with a batch size of {args.batch_size}.")
        ...
        avg_bleu, avg_rouge, avg_loss = trainer.evaluate(dev_loader)
        logging.info(f'Validation - Bleu: {avg_bleu:.3f}, Rouge: {avg_rouge:.3f}, Loss: {avg_loss:.3f}')